...
\begin{multline}
g\brak{ax+b(1-x)} \leqslant xg(a)+\brak{1-x}g(b), 
\\
...
\end{align}
is convex.
\item Find the equation of the line joining the points $\brak{a,e^{as}}$ and $\brak{b,e^{bs}}$.
\item ({\em Jensen's Inequality}) Show that
\begin{align}
g\brak{E\sbrak{X}} \leqslant E\sbrak{g(X)}
\end{align}
\item Show that $\Pr\brak{\abs{S_n-\mu}\geqslant \varepsilon}$
\end{enumerate}
\begin{align}
...
\textbf{ how about the tail problitity} $\Pr(|\frac{s_n}{n}-\mu \geqslant \varepsilon)?$\\
